...
class StockControllerIntegrationTest {
  ...
  private final ArticleReqDTO articleReqDTO = ArticleReqDTO.builder().name(ConsUtils.TEST_NAME)
                                                .description(ConsUtils.VALID_DESC).price(BigDecimal.TEN).quantity(ConsUtils.LONG_1)
                                                .categoryIds(Set.of(ConsUtils.LONG_1)).brandId(ConsUtils.LONG_1).build();
  private final BrandReqDTO brandReqDTO = BrandReqDTO.builder().name(ConsUtils.TEST_NAME)
                                            .description(ConsUtils.VALID_DESC).build();
  private final CategoryReqDTO categoryReqDTO = CategoryReqDTO.builder().name(ConsUtils.TEST_NAME)
                                                  .description(ConsUtils.VALID_DESC).build();
  private final CategoryReqDTO categoryReqDTO2 = CategoryReqDTO.builder().name(ConsUtils.LAST_TEST_NAME)
                                                   .description(ConsUtils.VALID_DESC).build();
  private static final ItemQuantityDTO ITEM_QUANTITY_DTO = ItemQuantityDTO.builder().articleId(ConsUtils.LONG_1)
                                                             .quantity(ConsUtils.LONG_1).build();
  private static final ItemQuantityDTO ITEM_QUANTITY_DTO_2 = ItemQuantityDTO.builder().articleId(ConsUtils.LONG_2)
                                                               .quantity(ConsUtils.LONG_2).build();
  ...
  void Should_Get201OnBrandCategory_When_ValidPayload() throws Exception {
    sentPostToCreateEntity(mapper.writeValueAsString(categoryReqDTO),
      ConsUtils.builderPath().withCategories().build()).andExpect(status().isCreated());
  }
  ...
  void Should_ThrowsException_When_CategoryAlreadyExists() throws Exception {
    ...
    sentPostToCreateEntity(categoryJSON, ConsUtils.builderPath().withCategories().build())
      .andExpect(jsonPath(ConsUtils.FIELD_MESSAGE)
                   .value(Category.class.getSimpleName() + ExceptionResponse.ENTITY_ALREADY_EXISTS));
  }

  @Test
  void Should_GetCorrectObjectFromPageResponse_When_GetAllCategoriesPageable() throws Exception {
    sentPostToCreateEntity(mapper.writeValueAsString(categoryReqDTO),
      ConsUtils.builderPath().withCategories().build());

    assertValidPageDTOFormResponseFromResult(
      mockMvc.perform(MockMvcRequestBuilders.get(ConsUtils.builderPath().withCategories().build())),
      ConsUtils.INTEGER_1);
  }
  ...
  void Should_GetAllCategoriesByName_When_ValidNames() throws Exception {
    ...
    mockMvc.perform(get(ConsUtils.builderPath().withCategoriesByName().build())
                      .params(buildParams(Map.of(ConsUtils.NAMES_PARAM, ConsUtils.TEST_NAME)))).andExpect(status().isOk())
  ...
  void Should_Get201OnBrandCreation_When_ValidPayload() throws Exception {
    sentPostToCreateEntity(mapper.writeValueAsString(brandReqDTO), ConsUtils.builderPath().withBrands().build())
      .andExpect(status().isCreated());
  }
  ...
  void Should_ThrowsException_When_BrandAlreadyExists() throws Exception {
    ...
    sentPostToCreateEntity(brandJSON, ConsUtils.builderPath().withBrands().build())
      .andExpect(jsonPath(ConsUtils.FIELD_MESSAGE)
                   .value(Brand.class.getSimpleName() + ExceptionResponse.ENTITY_ALREADY_EXISTS));
  }
  ...
  void Should_GetCorrectObjectFromPageResponse_When_GetAllBrandsPageable() throws Exception {
    ...
    assertValidPageDTOFormResponseFromResult(
      mockMvc.perform(MockMvcRequestBuilders.get(ConsUtils.builderPath().withBrands().build())),
      ConsUtils.INTEGER_1);
  }
  ...
  void Should_ThrowsException_When_CategoryOrBrandDoesNotExists() throws Exception {
    sentPostToCreateEntity(mapper.writeValueAsString(articleReqDTO), ConsUtils.builderPath().withArticles().build())
      .andExpect(jsonPath(ConsUtils.FIELD_MESSAGE)
                   .value(ExceptionResponse.ERROR_PROCESSING_OPERATION + Article.class.getSimpleName()));
  }
  ...
  void Should_GetCorrectObjectFromPageResponse_When_GetAllArticlesCategoryAscAndDesc() throws Exception {
    sentPostToCreateEntity(mapper.writeValueAsString(categoryReqDTO),
      ConsUtils.builderPath().withCategories().build()).andExpect(status().isCreated());
    sentPostToCreateEntity(mapper.writeValueAsString(categoryReqDTO2),
      ConsUtils.builderPath().withCategories().build()).andExpect(status().isCreated());
    sentPostToCreateEntity(mapper.writeValueAsString(brandReqDTO), ConsUtils.builderPath().withBrands().build())
      .andExpect(status().isCreated());
    ...
    ResultActions res = mockMvc
                          .perform(MockMvcRequestBuilders.get(ConsUtils.builderPath().withArticles().build())
                                     .param(ConsUtils.COLUMN_PARAM, ConsUtils.CATEGORIES_PARAM_VALUE)
  ...
  void Should_ThrowsException_When_InvalidToken() throws Exception {
    mockMvc.perform(MockMvcRequestBuilders.post(ConsUtils.SUPPLY_TEST_URL).header(ConsUtils.AUTHORIZATION,
      ConsUtils.BEARER + ConsUtils.SPACE)).andExpect(status().isUnauthorized());
  }

  @Test
  void Should_ThrowsException_When_NotValidRoleOnToken() throws Exception {
    mockMvc.perform(MockMvcRequestBuilders.post(ConsUtils.SUPPLY_TEST_URL).header(ConsUtils.AUTHORIZATION,
      ConsUtils.BEARER + getClientToken())).andExpect(status().isForbidden());
  }
  ...
  void Should_ThrowsException_When_NotValidArticlesOnStock() throws Exception {
    ...
    postSupplyWithAuxDepotToken().andExpect(status().isBadRequest())
      .andExpect(jsonPath(ConsUtils.FIELD_MESSAGE)
                   .value(ExceptionResponse.ERROR_PROCESSING_OPERATION + Article.class.getSimpleName()))
  ...
  void Should_ThrowsException_When_ArticleNotFound() throws Exception {
    ...
    mockMvc.perform(post(ConsUtils.builderPath().withCart().withArticles().build())
                      .contentType(MediaType.APPLICATION_JSON).content(mapper.writeValueAsString(itemsReqDTO))
  ...
  void Should_ThrowsException_When_NotEnoughStockForCart() throws Exception {
    ...
    itemsReqDTO.setItems(
      Set.of(ItemQuantityDTO.builder().articleId(ConsUtils.LONG_1).quantity(ConsUtils.LONG_10).build()));
    mockMvc.perform(post(ConsUtils.builderPath().withCart().withArticles().build())
                      .contentType(MediaType.APPLICATION_JSON).content(mapper.writeValueAsString(itemsReqDTO))
                      .header(ConsUtils.AUTHORIZATION, ConsUtils.BEARER + getClientToken()))
      .andExpect(jsonPath(ConsUtils.FIELD_MESSAGE).value(ExceptionResponse.STOCK_WILL_BE_AVAILABLE_SOON))
      .andExpect(jsonPath(ConsUtils.FIELD_ID_PATH)
                   .value(String.format(ExceptionResponse.NOT_ENOUGH_STOCK, ConsUtils.LONG_1)))
  ...
  void Should_ThrowsException_When_NotValidConstraintArticleCategoryQuantity() throws Exception {
    ...
    mockMvc.perform(post(ConsUtils.builderPath().withCart().withArticles().build())
                      ...
                      .header(ConsUtils.AUTHORIZATION, ConsUtils.BEARER + getClientToken()))
      .andExpect(jsonPath(ConsUtils.FIELD_MESSAGE)
                   .value(ExceptionResponse.FAILED_CONSTRAINT_FOR_ARTICLE_CATEGORY))
  ...
  void Should_Get200_When_ValidPayload() throws Exception {
    ...
    mockMvc.perform(post(ConsUtils.builderPath().withCart().withArticles().build())
                      .contentType(MediaType.APPLICATION_JSON).content(mapper.writeValueAsString(itemsReqDTO))
                      .header(ConsUtils.AUTHORIZATION, ConsUtils.BEARER + getClientToken())).andExpect(status().isOk());
  }
  ...
  void Should_GetAllCartItems_When_ValidParams() throws Exception {
    ...
    mockMvc.perform(
        get(ConsUtils.builderPath().withCart().withArticles().withArticlesIds().build(), ConsUtils.LONG_1)
          .header(ConsUtils.AUTHORIZATION, ConsUtils.BEARER + getClientToken()))
      .andExpect(jsonPath(ConsUtils.FIELD_TOTAL_ELEMENTS).value(ConsUtils.LONG_1)).andExpect(status().isOk());
  }
  ...
  void Should_Return200_When_IdsNotFound() throws Exception {
    ...
    mockMvc.perform(
        get(ConsUtils.builderPath().withCart().withArticles().withArticlesIds().build(), ConsUtils.LONG_10)
          .header(ConsUtils.AUTHORIZATION, ConsUtils.BEARER + getClientToken()))
  ...
  void Should_Return200_When_FilterByIdAndCategoryAndBrandNameASC() throws Exception {
    ...
    mockMvc.perform(
        get(ConsUtils.builderPath().withCart().withArticles().withArticlesIds().build(), ConsUtils.LONG_1)
          .params(buildParams(Map.of(ConsUtils.COLUMNS_PARAM,
            ConsUtils.CATEGORIES_PARAM_VALUE + ConsUtils.COMMA_DELIMITER
              + ConsUtils.BRAND_PARAM_VALUE)))
          .header(ConsUtils.AUTHORIZATION, ConsUtils.BEARER + getClientToken()))
  ...
  @Test
  @DirtiesContext(methodMode = DirtiesContext.MethodMode.BEFORE_METHOD)
  void Should_FilterByBrandAndOrCategoryName_When_ParamIsPresent() throws Exception {
    createArticle();

    mockMvc.perform(
        get(ConsUtils.builderPath().withCart().withArticles().withArticlesIds().build(), ConsUtils.INTEGER_1)
          .params(buildParams(Map.of(ConsUtils.BRAND_NAME, ConsUtils.TEST_NAME, ConsUtils.CATEGORY_NAME,
            ConsUtils.TEST_NAME)))
          .header(ConsUtils.AUTHORIZATION, ConsUtils.BEARER + getClientToken()))
      .andExpect(status().isOk())
      .andExpect(jsonPath(ConsUtils.FIELD_TOTAL_ELEMENTS).value(ConsUtils.INTEGER_1));
    ...
    mockMvc.perform(
        get(ConsUtils.builderPath().withCart().withArticles().withArticlesIds().build(), ConsUtils.INTEGER_1)
          .params(buildParams(Map.of(ConsUtils.BRAND_NAME, ConsUtils.CATEGORY)))
          .header(ConsUtils.AUTHORIZATION, ConsUtils.BEARER + getClientToken()))
      .andExpect(status().isOk())
      .andExpect(jsonPath(ConsUtils.FIELD_TOTAL_ELEMENTS).value(ConsUtils.INTEGER_0));
  }
  ...
  void Should_ThrowsAnException_When_InvalidSortDirection() throws Exception {
    getAllPageableEntities(ConsUtils.builderPath().withBrands().build(),
      buildParams(Map.of(ConsUtils.DIRECTION_PARAM, ConsUtils.NON_EXISTING_DIRECTION_COLUMN)))
      .andExpect(status().isBadRequest())
    ...
    getAllPageableEntities(ConsUtils.builderPath().withCategories().build(),
      buildParams(Map.of(ConsUtils.DIRECTION_PARAM, ConsUtils.NON_EXISTING_DIRECTION_COLUMN)))
      .andExpect(status().isBadRequest())
      .andExpect(jsonPath(ConsUtils.FIELD_MESSAGE).value(ExceptionResponse.NOT_VALID_PARAM))
      .andExpect(jsonPath(ConsUtils.FIELD_ERROR, Matchers.aMapWithSize(1)))
    ...
    getAllPageableEntities(ConsUtils.builderPath().withArticles().build(),
      buildParams(Map.of(ConsUtils.DIRECTION_PARAM, ConsUtils.NON_EXISTING_DIRECTION_COLUMN)))
      .andExpect(status().isBadRequest())
      .andExpect(jsonPath(ConsUtils.FIELD_MESSAGE).value(ExceptionResponse.NOT_VALID_PARAM))
      .andExpect(jsonPath(ConsUtils.FIELD_ERROR, Matchers.aMapWithSize(1)))
  ...
  private void assertValidPageDTOFormResponseFromResult(ResultActions res, Integer elements) throws Exception {
    ...
    res.andExpect(jsonPath(ConsUtils.BASE_MATCHER, Matchers.aMapWithSize(ELEMENTS_FOR_PAGE)))
      .andExpect(status().isOk()).andExpect(jsonPath(ConsUtils.FIELD_TOTAL_ELEMENTS).value(elements))
  ...
  private ResultActions sentPostToCreateEntity(String dto, String url) throws Exception {
    return mockMvc.perform(MockMvcRequestBuilders.post(url).content(dto).contentType(MediaType.APPLICATION_JSON));
  }

  private String getAuxDepotToken() {
    CustomUserDetails userDetail = new CustomUserDetails(ConsUtils.TEST_NAME, ConsUtils.PASSWORD,
      Set.of(new SimpleGrantedAuthority(ConsUtils.ROLE.concat(ConsUtils.AUX_DEPOT))), ConsUtils.LONG_1);
    return JwtUtils
             .createToken(new UsernamePasswordAuthenticationToken(userDetail, null, userDetail.getAuthorities()));
  }

  private String getClientToken() {
    CustomUserDetails userDetail = new CustomUserDetails(ConsUtils.TEST_NAME, ConsUtils.PASSWORD,
      Set.of(new SimpleGrantedAuthority(ConsUtils.ROLE.concat(ConsUtils.CLIENT))), ConsUtils.LONG_1);
    return JwtUtils
             .createToken(new UsernamePasswordAuthenticationToken(userDetail, null, userDetail.getAuthorities()));
  }

  private ResultActions getAllPageableEntities(String url, MultiValueMap<String, String> params) throws Exception {
    return mockMvc.perform(MockMvcRequestBuilders.get(url).params(params).contentType(MediaType.APPLICATION_JSON));
  }
